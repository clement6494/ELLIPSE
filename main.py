...
def percentage(contract):
    ...
    '''
    print("\nCities with the most bike stations:")
    sorted_villes = sorted(villes.items(), key=lambda x: x[1], reverse=True)
    for ville, count in sorted_villes[:10]:
    print(f"{ville}: {count} stations")   
    '''
...
def main():
    ...
    choice=1
    while choice!=0:
        print("1. Afficher les parts de vélos mécaniques et électriques pour une ville")
        ...
        if choice==1:
            ...
            villes = cities()
            for ville in villes:
                villes[ville] = veloCount(ville)
            sorted_villes = sorted(villes.items(), key=lambda x: x[1], reverse=True)
            for ville, count in sorted_villes[:nb_villes]:
